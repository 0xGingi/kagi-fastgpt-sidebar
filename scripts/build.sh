...
cd dist/chrome
zip -r ../kagi-fastgpt-chrome-v1.0.8.zip . > /dev/null
cd ../..
...
if [ -z "$WEB_EXT_API_KEY" ] || [ -z "$WEB_EXT_API_SECRET" ]; then
    ...
    zip -r ../kagi-fastgpt-firefox-v1.0.8-unsigned.xpi . > /dev/null
    cd ../..
    echo ""
    echo "Package files created:"
    echo "   Chrome:  dist/kagi-fastgpt-chrome-v1.0.8.zip"
    echo "   Firefox: dist/kagi-fastgpt-firefox-v1.0.8-unsigned.xpi (UNSIGNED - for manual upload)"
    echo ""
    ...
    if command -v web-ext &> /dev/null; then
        ...
        if web-ext sign --channel=listed --amo-metadata=../../amo-metadata.json --api-key="$WEB_EXT_API_KEY" --api-secret="$WEB_EXT_API_SECRET"; then
            ...
            echo "Package files created:"
            echo "   Chrome:  dist/kagi-fastgpt-chrome-v1.0.8.zip"
            echo "   Firefox: dist/kagi-fastgpt-firefox-v1.0.8-unsigned.xpi (fallback)"
            echo ""
        ...
        echo "Package files created:"
        echo "   Chrome:  dist/kagi-fastgpt-chrome-v1.0.8.zip"
        echo "   Firefox: dist/kagi-fastgpt-firefox-v1.0.8-unsigned.xpi (UNSIGNED)"
        echo ""
